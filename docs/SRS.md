...
* **Trace 14: In-Application Ticketing & Query Escalation:** **The Tickets service (`apps/tickets/main.py`) enforces strict lifecycle transition controls, optimistic locking, auditor-blocking access constraints, and site-scoped isolation for tickets, comments, and audit trails. Automated notifications on assignment, comments, or transitions, and background SLA escalations, are verified across `tests/test_tickets_service.py`, `tests/test_tickets_escalation.py`, and `tests/test_tickets_notifications_integration.py`.**
